...
'''
def rhs(s):
    return np.transpose(allbits).dot(np.kron(s,np.identity(8)))
'''
def accuracy(s):
...
def accuracy(s):
    ...
    dist = np.zeros((n_sessions,))
    
    for i in range(n_sessions):
        temp = RHS[:,(0 + 8*i):(8 + 8*i)]
        
        if (np.linalg.norm(temp) < 0.00001):
            dist[i] = 15
        else:
            difference = np.subtract(temp.dot(LHS),LHS.dot(temp))
            dist[i] = np.linalg.norm(difference)
...
for Case in range(num_cases):
    ...
    if new_input:
        ...
        [init_training, LHS, allbits, reset_state, n] = eng.pretrain_general(nargout = 5)
        
        
        coeffs_size = 18#int( n*(n+1)/2 )
        n_actions = 2*coeffs_size + 1   # number of actions
        
        LHS_size = int(2**(n-aux))
    ...
    if aux:
        comig_cost = 2*best_num_of_comigs*aux
    else:
        comig_cost = best_num_of_comigs
