/* lrslong.h      (lrs __int32_t integer arithmetic library              */
...
/*************/
#ifndef B128
typedef __int32_t lrs_mp[1];		/* type lrs_mp holds one __int32_t integer */
typedef __int32_t *lrs_mp_t;
typedef __int32_t **lrs_mp_vector;
typedef __int32_t ***lrs_mp_matrix;
#else
typedef __int32_t lrs_mp[1];	/* type lrs_mp holds one 128-bit integer */
typedef __int32_t *lrs_mp_t;
typedef __int32_t **lrs_mp_vector;
typedef __int32_t ***lrs_mp_matrix;
#endif
...
extern __int32_t lrs_digits;		/* max permitted no. of digits   */
extern __int32_t lrs_record_digits;		/* this is the biggest acheived so far.     */
...
__int32_t lrs_mp_init (__int32_t dec_digits, FILE * lrs_ifp, FILE * lrs_ofp);	/* max number of decimal digits, fps   */

#define lrs_alloc_mp(a)	
#define lrs_clear_mp(a)

lrs_mp_t lrs_alloc_mp_t();                      /* dynamic allocation of lrs_mp                  */
lrs_mp_vector lrs_alloc_mp_vector (__int32_t n);	/* allocate lrs_mp_vector for n+1 lrs_mp numbers */
lrs_mp_matrix lrs_alloc_mp_matrix (__int32_t m, __int32_t n);	/* allocate lrs_mp_matrix for m+1 x n+1 lrs_mp   */

void lrs_clear_mp_vector (lrs_mp_vector a, __int32_t n);
void lrs_clear_mp_matrix (lrs_mp_matrix a, __int32_t m, __int32_t n);
...
__int32_t compare (lrs_mp a, lrs_mp b);	/* a ? b and returns -1,0,1 for <,=,> */
void gcd (lrs_mp u, lrs_mp v);	/* returns u=gcd(u,v) destroying v                */
void mptodouble (lrs_mp a, double *x);	/* convert lrs_mp to double                       */
__int32_t mptoi (lrs_mp a);		/* convert lrs_mp to __int32_t integer */
#ifdef PLRS
string spmp (char name[], lrs_mp a);	/* print the __int32_t precision integer a             */
...
__int32_t plrs_readrat (lrs_mp Na, lrs_mp Da, const char * rat);	/* take a rational number and convert to lrs_mp   */
#endif
void pmp (char name[], lrs_mp a);	/* print the __int32_t precision integer a             */
void prat (char name[], lrs_mp Nt, lrs_mp Dt);	/* reduce and print  Nt/Dt                        */
void readmp (lrs_mp a);		/* read an integer and convert to lrs_mp          */
__int32_t readrat (lrs_mp Na, lrs_mp Da);	/* read a rational or int and convert to lrs_mp   */
...
__int32_t atos (char s[]);		/* convert s to integer                           */
__int32_t comprod (lrs_mp Na, lrs_mp Nb, lrs_mp Nc, lrs_mp Nd);	/* +1 if Na*Nb > Nc*Nd,-1 if Na*Nb > Nc*Nd else 0 */
void divrat (lrs_mp Na, lrs_mp Da, lrs_mp Nb, lrs_mp Db, lrs_mp Nc, lrs_mp Dc);
						       /* computes Nc/Dc = (Na/Da) /( Nb/Db ) and reduce */
void getfactorial (lrs_mp factorial, __int32_t k);	/* compute k factorial in lrs_mp                  */
void linrat (lrs_mp Na, lrs_mp Da, __int32_t ka, lrs_mp Nb, lrs_mp Db, __int32_t kb, lrs_mp Nc, lrs_mp Dc);
...
void mulrat (lrs_mp Na, lrs_mp Da, lrs_mp Nb, lrs_mp Db, lrs_mp Nc, lrs_mp Dc);
						       /* computes Nc/Dc=(Na/Da)*(Nb/Db) and reduce      */
__int32_t myrandom (__int32_t num, __int32_t nrange);	/* return a random number in range 0..nrange-1    */
...
void reducearray (lrs_mp_vector p, __int32_t n);	/* find gcd of p[0]..p[n-1] and divide through by */
void scalerat (lrs_mp Na, lrs_mp Da, __int32_t ka);	/* scales rational by ka                          */

/********************************/
/* Matrix and vector allocation */
/********************************/
void lrs_clear_mp_vector (lrs_mp_vector p, __int32_t n);
lrs_mp_vector lrs_alloc_mp_vector(__int32_t n);
void lrs_clear_mp_vector(lrs_mp_vector p, __int32_t n);
lrs_mp_matrix lrs_alloc_mp_matrix(__int32_t m, __int32_t n);
void lrs_clear_mp_matrix(lrs_mp_matrix p, __int32_t m, __int32_t n);
__int32_t lrs_mp_init(__int32_t dec_digits, FILE *fpin, FILE *fpout);


/* how big are numbers? */
extern __int32_t lrs_digits;		/* max permitted no. of digits   */
extern __int32_t lrs_record_digits;	/* this is the biggest achieved so far. */

/**********************************/
/* Miscellaneous functions        */
/******************************** */

void lrs_getdigits (__int32_t *a, __int32_t *b);	/* send digit information to user                         */

void stringcpy (char *s, char *t);	/* copy t to s pointer version                            */

void *calloc ();
void *malloc ();
void *xcalloc (__int32_t n, __int32_t s, __int32_t l, char *f);
