...
int lrs_main(__int32_t argc, char *argv[]){
    //matrix size (m_A+1) * (d+1), number of iterations numit
    int i, j, k, rep, r=3, s=14, m_A=1000, d=1000, numit=10000;
    volatile __int32_t *det, *Ars, *Nt, *Ns;

    clock_t init, t;
    
    for(j = 0; j<=m_A; j++){
        for(k = 0; k<=d; k++){
            A[j][k] = (__int32_t*) malloc(sizeof(__int32_t));
            B[j][k] = (__int32_t*) malloc(sizeof(__int32_t));
            *A[j][k] = 3* (k+j);
        }
    }

    Ars = (__int32_t*) malloc(sizeof(__int32_t));
    *Ars = *A[r][s];
    det = (__int32_t*) malloc(sizeof(__int32_t));
    Nt = (__int32_t*) malloc(sizeof(__int32_t));
    Ns = (__int32_t*) malloc(sizeof(__int32_t));
    *det = 3;
    ...
    for(rep = 0; rep <= numit; rep++){
        ...
        if(rep%(numit/100
            ...
            t = clock() - init;
            printf("B[56][27]= %d, %d rep, %f seconds\n", *B[56][27], rep, (float) t / CLOCKS_PER_SEC);
        }
    }
    return 0;
}
