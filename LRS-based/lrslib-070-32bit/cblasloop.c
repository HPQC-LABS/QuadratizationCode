...
/*******************************************************/
int B[2048*100], *ptr[2048][100], *A[2048][100];

int lrs_main(__int32_t argc, char *argv[]){
    //matrix size (m_A+1) * (d+1), number of iterations numit, has copy overhead if iscopy==1
    int i, j, k, rep, r=3, s=14, m_A=2047, d=66, numit=200000, iscopy=1;
    ...
    __int16_t Ars, *Ais, *Arj;
    //B = (int*) malloc((m_A+1)*(d+1)*sizeof(int));
    ...
    for(j = 0; j<=m_A; j++){
        for(k = 0; k<=d; k++){
            ptr[j][k] = (int*) malloc(sizeof(int));
            A[j][k] = (int*) malloc(sizeof(int));
            *ptr[j][k] = 3*(j+k);
            B[k+j*(d+1)] = 3*(j+k);
        }
    }
    Ars = (__int16_t) A[s+r*(d+1)];
    
    init = dsecnd();
    for(rep = 0; rep <= numit; rep++){
        if(iscopy==1){
            for (j = 0; j<=m_A; j++){
                for(k = 0; k<=d; k++){
                    B[k+j*(d+1)] = *(ptr[j][k]);
        ...
        for (i = 0 ; i<= m_A; i++){
            Ais[i] = (__int16_t) B[s+i*(d+1)];
        }
        for (j = 0 ; j<=d; j++){
            Arj[j] = (__int16_t) B[j+r*(d+1)];
        }
        //B = -1/det * dAis * dArj  + dArs/det * A
        //cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
        // m_A+1, d+1, 1, -1.0/ddet, dAis, 1, dArj, d+1, 1.0*dArs/ddet, dB, d+1);
        cblas_gemm_s16s16s32 (CblasRowMajor, CblasNoTrans, CblasNoTrans, CblasFixOffset,
                              m_A+1, d+1, 1, -1.0/det, Ais, 1, 0, Arj, d+1, 0, 1.0*Ars/det, B, d+1, 0);
        if(iscopy){
            for (j = 0; j<=m_A; j++){
                for(k = 0; k<=d; k++){
                    *A[j][k] = B[k+j*(d+1)];
        ...
        if(rep%(numit/100)==0){
            t = dsecnd() - init;
            printf("B[56][27]= %d, %d rep, %f seconds\n", B[27+56*(d+1)], rep, t);
        }
    }
    return 0;
}
